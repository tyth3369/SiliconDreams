...
- [x] 可复用公司对比模板与指标口径字典
- [x] 财务时间序列图、制程收入结构图和 Capex 强度图
- [x] Watchlist 与事件时间线
- [x] Web 搜索缓存、官方域名优先和抓取快照
- [x] 端到端浏览器测试与性能基准
- [x] 术语关系规范化：实体、别名、关系类型、闭合边审计
...
- [x] 安装、备份、恢复、升级和安全文档
- [x] 发布候选冻结与验收清单
- [x] 生产依赖漏洞门禁、限时例外登记与 Chroma 嵌入式架构回归
- [x] 版本化 GHCR 镜像、供应链 provenance 与无凭据公网部署验收器

## 发布门槛

每个里程碑必须同时满足：Ruff 通过、全部 pytest 通过、`pip-audit`（仅允许已登记且攻击面不可达的限时例外）通过、secrets 未跟踪、真实浏览器关键路径通过、检索与引用基准通过、devlog 已更新。GitHub push 只能在用户明确授权后进行。
